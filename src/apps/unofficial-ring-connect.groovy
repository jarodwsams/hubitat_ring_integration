...
- The last GUID is the access token created by this app using OAuth that IFTTT will use to authenticate to Hubitat. <b>DO NOT</b> disclose this to anybody. It is like a password.""")
    ...
    Set<String> dingableDnis = null

    section('<b style="font-size: 22px;">Body Payloads for Motion Events</b>') {
      if (dingables) {
        dingableDnis = (dingables*.deviceNetworkId).toSet()
        paragraph(dingables.collect { ChildDeviceWrapper dingable ->
          return "<u>" + makeChildDeviceLink(dingable) + "</u>: " + JsonOutput.toJson([kind: 'motion', motion: true, id: dingable.deviceNetworkId])
        }.join('\n\n'))
      } else {
        paragraph("No installed devices support motion events. See below for list of installed child devices that aren't ringlable or dingable")
      }
    }
    section('<b style="font-size: 22px;">Body Payloads for Ring Events</b>') {
      List ringables = dingables?.findAll { ChildDeviceWrapper dingable -> RINGABLES.contains(dingable.getDataValue("kind")) }

      if (ringables) {
        paragraph(ringables.collect { ChildDeviceWrapper ringable ->
          "<u>" + makeChildDeviceLink(ringable) + "</u>: " + JsonOutput.toJson([kind: 'ding', motion: false, id: ringable.deviceNetworkId])
        }.join('\n\n'))
      } else {
        paragraph("No installed devices support ring events. See below for list of installed child devices that aren't ringlable or dingable")
      }
    }
    ...
    List<ChildDeviceWrapper> nonDingableChildren = getChildDevices().findAll { ChildDeviceWrapper child -> !dingableDnis.contains(child.deviceNetworkId) }
...
<li>Your hub will be under additional load to pull the images. This is likely the primary factor to consider when deciding whether or not to enable snapshot polling and usage. This additional load will be even higher if dashboards are left open continuously</li>
    ...
    section('<h2>Configuration</h2>') {
      ...
      for (final String snappableDNI in state.snappables.keySet()) {
        ChildDeviceWrapper child = getChildDevice(snappableDNI)
        if (child != null) {
          msg += makeChildDeviceLink(child) + " (Opted <b>${state.snappables?.get(snappableDNI) ? 'in to' : 'out of'}</b> snapshot polling)\n"
        } else {
...
<li>Click the "Add Tile" button.</li>
      ...
      Set<String> enabledSnappebles = getEnabledSnappables()

      if (enabledSnappebles.size() == 0) {
...
void periodicMaintenance() {
  ...
  // Remove entries from state.snappables that have the old 'RING-' DNI prefix
  ...
  for (final String snappableDNI in state.snappables.keySet()) {
    getChildDevice(snappableDNI)?.refresh()
  }
}
